...
class MarketRadar:
    def __init__(self):
        self.history = {}
        self.signals = []
        self.stats_top5 = {}     # 15dk'da bir sıfırlanacak
        self.stats_counters = {}  # 15dk'da bir sıfırlanacak
        self.lock = threading.RLock()
        self.last_heartbeat = 0  
        self.total_pairs = 0
        # 15 dakikalık blok takibi (0, 1, 2, 3 nolu bloklar)
        self.last_reset_block = datetime.now().minute // 15

    def check_resets(self):
        """Her 15 dakikada bir (00, 15, 30, 45) tüm istatistikleri sıfırlar"""
        now = datetime.now()
        current_block = now.minute // 15
        
        if current_block != self.last_reset_block:
            with self.lock:
    ...
    def check_logic(self, symbol, now):
        ...
        # 2. CONFIRMED TREND (15dk bekleme/onay mekanizması)
    ...
    def add_signal(self, symbol, price, chg_main, chg_ref, vol, s_type, mode):
        ...
        with self.lock:
            ...
            # 15 Dakikalık İstatistikleri Güncelle
            ...
            # Anlık sayıyı satıra sabitle (Snapshots)
...
# --- UI ---
st.set_page_config(layout="wide", page_title="SinyalEngineer 15m Radar")
...
h1.title("🛡️ 15m Scalp Radar")
h1.markdown('<p class="reset-timer">RESETS EVERY 15 MINUTES (00, 15, 30, 45) / 15 DAKİKADA BİR SIFIRLANIR</p>', unsafe_allow_html=True)

status_html = '<span class="status-live">● SYSTEM LIVE</span>' if (time.time() - radar.last_heartbeat) < 15 else '<span class="status-offline">● OFFLINE</span>'
...
while True:
    with placeholder_side.container():
        st.subheader("🔥 Top 5 (15m)")
        with radar.lock:
            # AttributeError önleyici güvenli erişim
            s_top = getattr(radar, 'stats_top5', {})
            sorted_stats = sorted(s_top.items(), key=lambda x: x[1]['PUMP'] + x[1]['DUMP'], reverse=True)[:5]
    ...
    with placeholder_main.container():
        with radar.lock:
            ...
            if display_data:
                html = "<table><tr><th>Time</th><th>Symbol (15m ↑/↓)</th><th>Price</th><th>Mtm.</th><th>15m Ref</th><th>Vol</th><th>Status</th><th>Type</th></tr>"
                for row in display_data:
                    sym = row.get('Symbol'); p_count = row.get('SnapP'); d_count = row.get('SnapD')
                    tv_url = f"https://www.tradingview.com/chart/?symbol=BINANCE:{sym}USDT.P"
                    chg = row.get('Chg'); ref = row.get('Ref'); vol = row.get('Vol'); p_type = row.get('P/D'); mode = row.get('Mode')
            ...
            else: st.info("Piyasa taranıyor... (Trend Onayı için 15dk veri birikmesi bekleniyor) 🔍")
    time.sleep(1)
